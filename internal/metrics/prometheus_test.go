...
func TestQueryDeviceMetricsParsesPrometheusResponses(t *testing.T) {
	...
	metrics, err := client.QueryDeviceMetrics(context.Background(), "instance", []string{"172.28.10.11", "172.28.10.10"}, promMetrics)
	if err != nil {
	...
	// Verify all queries reference both target IPs (dots are double-escaped for PromQL regex)
	for _, query := range queries {
		if !strings.Contains(query, `172\\.28\\.10\\.10`) || !strings.Contains(query, `172\\.28\\.10\\.11`) {
			t.Fatalf("query %q missing target IPs", query)
		}
		if !strings.Contains(query, "instance") {
			t.Fatalf("query %q missing label name 'instance'", query)
		}
	}
...
func TestQueryDeviceMetricsEmptyResultsReturnNilPointers(t *testing.T) {
	...
	metrics, err := client.QueryDeviceMetrics(context.Background(), "instance", []string{"172.28.10.10"}, promMetrics)
	if err != nil {
		t.Fatalf("QueryDeviceMetrics returned error: %v", err)
	}
...
func TestQueryLinkMetricsParsesThroughputAndUtilization(t *testing.T) {
	...
	client := NewPromClient(server.URL, server.Client())
	results, err := client.QueryLinkMetrics(context.Background(), "instance", []string{"172.28.10.10"})
	if err != nil {
