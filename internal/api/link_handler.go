...
// enrichedLinkResponse extends a Link with interface speed and oper_status so
...
// Returns links enriched with interface speed and oper_status for edge rendering.
...
func (h *LinkHandler) HandleCreate(w http.ResponseWriter, r *http.Request) {
	...
	stored, created, err := h.linkRepo.CreateManualIdempotent(link, isBrowserLocalStorageMigration)
	if err != nil {
		writeError(w, http.StatusInternalServerError, "internal error", err)
		return
	}
