...
func TestProbeDiscoversNeighbors_CreatesUnmanagedPlaceholders(t *testing.T) {
	...
	// Check that a placeholder neighbor was created
	devices, err := deviceRepo.GetAll()
	if err != nil {
		t.Fatalf("GetAll failed: %v", err)
	}
	if len(devices) != 2 {
		t.Fatalf("expected 2 devices (1 managed + 1 placeholder), got %d", len(devices))
	}

	// Find the placeholder
	var placeholder *domain.Device
	for i := range devices {
	...
	if placeholder == nil {
		t.Fatal("expected unmanaged placeholder device")
	}
	if placeholder.Status != domain.DeviceStatusUnknown {
		t.Errorf("expected placeholder status unknown, got %s", placeholder.Status)
	}
	if placeholder.SysName != "switch2" {
		t.Errorf("expected placeholder sysName switch2, got %s", placeholder.SysName)
	}
