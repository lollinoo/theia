...
Restore upload is `multipart/form-data` with a `file` part ending in `.tar.gz`. `dry_run=true` validates and returns the report without staging. A normal successful request validates, extracts into private staging, persists a restart marker/status, returns the report with HTTP `200`, and then invokes the asynchronous restart handoff. After restart, the coordinator backs up the live database and applies PostgreSQL. The completion verifier then loads and verifies the credential keyring, runs migrations and credential rewrap, and revokes restored sessions before optional artifacts activate: first the device-backup directory, then `known_hosts`. A missing key or other verifier failure therefore occurs before optional artifact activation; if `known_hosts` activation later fails, the backup directory may already be live. Only a fully successful verification and optional-artifact sequence removes the restart marker and staging.
