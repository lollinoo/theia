...
export function Toolbar({
  ...
  return (
    <div className="topology-glass topology-floating-shadow absolute right-4 top-16 z-10 flex flex-col gap-1 rounded-[16px] p-1.5 transition-colors duration-200">
      {buttons.map((btn) => (
        <button
          key={btn.label}
          type="button"
          className={`relative flex h-11 w-11 items-center justify-center rounded-xl border border-transparent transition-[background-color,color,border-color,transform] duration-150 hover:-translate-y-0.5 hover:bg-surface-container focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus-ring focus-visible:ring-offset-2 focus-visible:ring-offset-bg ${'active' in btn && btn.active ? 'border-primary/35 bg-primary/12 text-primary' : 'text-on-bg-secondary hover:text-on-bg'}`}
          onClick={btn.onClick}
          title={btn.label}
        >
          {btn.icon}
