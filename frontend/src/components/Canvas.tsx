...
function buildTopologyEdges(
  ...
  return links
    ...
    .map((link) => {
      ...
      const data = buildEdgeData(link, devicesByID, onContextMenu);
...
export default function Canvas() {
  ...
  async function loadTopology() {
    ...
    try {
      ...
      const nextEdges = [
        ...buildTopologyEdges(fetchedLinks, devicesByID, nextNodes),
        ...buildManualEdges(loadManualEdges(), nextNodes, devicesByID, openEdgeMenu),
      ];
    ...
    <div className="relative h-full w-full bg-bg-canvas">
      ...
      <ReactFlow
        ...
        onNodeDragStop={(_event, node) => {
          ...
          const topologyEdgeIDs = new Set(topologyLinks.map((link) => link.id));
          const repositionedEdges = [
            ...buildTopologyEdges(topologyLinks, devicesByID, updatedNodes),
            ...edges.filter((edge) => !topologyEdgeIDs.has(edge.id)),
          ];
