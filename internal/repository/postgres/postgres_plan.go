...
// PostgresPlanCheck describes an EXPLAIN-based index usage assertion.
...
var defaultRepositoryPlanCheckRegistry = []repositoryPlanCheck{
	{
		...
		Query:         `SELECT id FROM devices WHERE sys_name_lookup = $1 AND sys_name_lookup != '' ORDER BY updated_at DESC, created_at DESC LIMIT 1`,
	...
	},
	{
		...
		ExpectedIndex: "idx_links_target_device_created_at",
	...
	},
	{
		...
		ExpectedIndex: "idx_topology_observations_remote_identity_protocol",
		Args:          []any{"device-a", "core-switch", "ether1", "ether10", "lldp"},
	},
	{
		Name:          "unresolved-neighbors-by-device",
		Query:         `SELECT id FROM unresolved_neighbors WHERE local_device_id = $1 AND resolved_at IS NULL ORDER BY protocol, remote_identity`,
		ExpectedIndex: "idx_unresolved_neighbors_local_device_id",
		Args:          []any{"device-a"},
	},
}
...
// DefaultPostgresPlanChecks returns the repository query plan checks used by the validation CLI.
...
// ValidatePostgresPlanChecks verifies that important PostgreSQL queries use their expected indexes.
