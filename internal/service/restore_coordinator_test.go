...
// TestRestoreCoordinatorApplyPendingRestore_TargetMismatchFailsBeforeSideEffects preserves runtime ownership fail-closed behavior.
func TestRestoreCoordinatorApplyPendingRestore_TargetMismatchFailsBeforeSideEffects(t *testing.T) {
	stateDir := t.TempDir()
	stagingDir := filepath.Join(stateDir, ".restore-staging")
	if err := os.MkdirAll(stagingDir, 0700); err != nil {
		t.Fatalf("creating staging dir: %v", err)
	...
	marker := newRestoreMarker(
		filepath.Join(stagingDir, "postgres.dump"),
		"",
		"",
		filepath.Join(stateDir, "other-state"),
		deviceBackupDir,
		knownHostsPath,
		"2026-06-04T00:00:00Z",
	)
	if err := writeRestoreMarker(markerPath, marker); err != nil {
		t.Fatalf("writing target-mismatched marker: %v", err)
	}

	externalCommandCalled := false
	stubExternalCommandsWithEnv(t, func(ctx context.Context, env []string, name string, args ...string) ([]byte, error) {
		externalCommandCalled = true
		t.Fatalf("external command %q should not run for a target-mismatched marker", name)
		return nil, nil
	})
	...
	terminatePostgresConnections = func(ctx context.Context, dsn string) error {
		terminationCalled = true
		t.Fatal("postgres connection termination should not run for a target-mismatched marker")
		return nil
	}
	t.Cleanup(func() { terminatePostgresConnections = originalTerminate })
	...
	if !errors.Is(err, errRestoreMarkerTargetMismatch) {
		t.Fatalf("ApplyPendingRestore() error = %v, want target mismatch", err)
	}
	if applied {
		t.Fatal("ApplyPendingRestore() applied = true, want false")
	}
	if externalCommandCalled {
		t.Fatal("external command ran before marker target mismatch failed closed")
	}
	if terminationCalled {
		t.Fatal("postgres termination ran before marker target mismatch failed closed")
	}
	if _, err := os.Stat(markerPath); err != nil {
		t.Fatalf("marker should remain after target mismatch failure: %v", err)
	}
	info, err := os.Stat(stagingDir)
	if err != nil {
		t.Fatalf("staging dir should remain after target mismatch failure: %v", err)
	}
	if !info.IsDir() {
		t.Fatal("staging path should remain a directory after target mismatch failure")
	}
}
